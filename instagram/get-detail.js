...
const urls = require('./result-data/instagram-urls.json')
...
async function saveHtmlToFile(html, filename) {
  ...
  try {
    ...
    console.error(`🚨 保存HTML失败: ${error.message}`)
  }
}
...
async function main() {
  ...
  try {
    ...
    const filteredUrls = urls.filter(
      (url) => url && url.trim() !== '' && url !== 'TODO'
    )

    if (filteredUrls.length === 0) {
      console.error(
        'urls.json文件中没有有效的URL，请在urls.json中添加Instagram URL'
      )
      return
    }
    ...
    for (let i = 0; i < filteredUrls.length; i++) {
      ...
      console.log(
        `\n=================== 第${batchNumber}批次 - 第${positionInBatch}/${Math.min(
          BATCH_SIZE,
          filteredUrls.length - Math.floor(i / BATCH_SIZE) * BATCH_SIZE
        )}个 (总进度: ${i + 1}/${filteredUrls.length}) ===================`
      )
